...
def test_create_warning_box_cancel(qtbot, app):
    ...
    def execute_warning_box():
        nonlocal result
        box = QMessageBox()
        result = app.create_warning_box(
            "Test Message", add_cancel_btn=True, custom_dialog=box
        )

    qtbot.waitUntil(
        execute_warning_box, timeout=5000
    )  # Add a timeout for the function to execute
