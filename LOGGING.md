...
This document describes the logging system implemented in the Bioimage Annotation Tool for Segmentation (BATS).

## Overview

Both the client and server components now have comprehensive logging support:

- **Client**: `bats_client/utils/logger.py`
- **Server**: `bats_server/utils/logger.py`
...
The client logger is automatically initialized in `bats_client/main.py`:

```python
from bats_client.utils.logger import setup_logger, get_logger

# Initialize main logger with file output
setup_logger(log_file=os.path.join(os.path.expanduser("~"), ".bats_client", "bats_client.log"))
...
The server logger is automatically initialized in `bats_server/main.py`:

```python
from bats_server.utils.logger import setup_logger, get_logger

# Initialize main logger with file output
setup_logger(log_file=os.path.join(os.path.expanduser("~"), ".bats_server", "bats_server.log"))
...
```python
from bats_client.utils.logger import get_logger
...
```python
from bats_server.utils.logger import get_logger
...
```
2024-02-02 10:30:45 - bats_client.main - INFO - Starting BATS Client...
2024-02-02 10:30:45 - bats_client.utils.bentoml_model - DEBUG - Attempting to connect to BentoML server at http://0.0.0.0:7010
```
...
```
2024-02-02 10:30:45 - bats_client.main - INFO - [main.py:52] - Starting BATS Client...
2024-02-02 10:30:45 - bats_client.utils.bentoml_model - DEBUG - [bentoml_model.py:25] - Attempting to connect to BentoML server at http://0.0.0.0:7010
```
...
- `bats_client/utils/logger.py` (for client)
- `bats_server/utils/logger.py` (for server)
