...
BATS includes a client and server side for using our Bioimage Annotation Tool for Segmentation. The client and server communicate via the `bentoml <https://www.bentoml.com/?gclid=Cj0KCQiApKagBhC1ARIsAFc7Mc6iqOLi2OcLtqMbGx1KrFjtLUEZ-bhnqlT2zWREE0x7JImhtNmKlFEaAvSSEALw_wcB>`_ library. 
There are currently two options available: running the server locally, or connecting to the running instance on the FZJ jusuf-cloud.
   Before continuing, you need to make sure that BATS server is running, either locally or on the cloud. See :ref:`BATS Client` for instructions on how to launch the server. **Note:** In order for this connection to succeed, you will need to have contacted the team developing BATS, so they can add your IP to the list of accepted requests.
...
~~~~~~~~~~~~~~~~~~~~~~
   ...
   .. image:: https://raw.githubusercontent.com/HelmholtzAI-Consultants-Munich/BATS/main/src/client/readme_figs/client_welcome_window.png
         :width: 400
         :height: 200
         :align: center
   ...
   This folder is intended to store all images of your dataset. These images may be accompanied by corresponding segmentations. If present, segmentation files should share the same filename as their associated image, appended with a suffix as specified in ``server/bats_server/config.cfg`` file (default: '_seg').
...
~~~~~~~~~~~~~~~~~~~~
   ...
   .. image:: https://raw.githubusercontent.com/HelmholtzAI-Consultants-Munich/BATS/main/src/client/readme_figs/client_data_overview_window.png
      :width: 500
      :height: 200
      :align: center
...
~~~~~~~~~~~~~~~~~~~~
   ...
   In BATS, we use  `napari <https://napari.org/stable>`_ for viewing our images and masks, adding, editing or removing labels. The newest version of BATS comes with an AI-assisted labelling feature, using meta SAM2 model, to enable even faster labelling! An example of the viewer can be seen below. 
...
- If the number of class labels in the class mask surpasses that defined in the ``--num-classes`` argument, a message appears to the user to correct the class mask. Note: the label ids don't need to follow a numeric order, can be 1,4,16, etc.
   ...
   .. image:: https://raw.githubusercontent.com/HelmholtzAI-Consultants-Munich/BATS/main/src/client/readme_figs/client_napari_viewer.png
      :width: 900
      :height: 500
      :align: center
...
The intended usage of BATS would include the following:
...
.. image:: https://raw.githubusercontent.com/HelmholtzAI-Consultants-Munich/BATS/main/src/client/readme_figs/napari_shortcuts.png
   :width: 600
   :height: 500
   :align: center
